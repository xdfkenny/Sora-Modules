...
// Extract episode entries from a season/episode-listing page.
function collectEpisodesFromSeason(seasonHtml, results) {
...
async function extractEpisodes(url) {
    ...
    try {
        ...
        const isMovie = decodedUrl.indexOf("فيلم") !== -1;
        const isEpisodePage = !isMovie && html.indexOf('class="watch"') !== -1;

        if (isEpisodePage) {
            // Episode page: the season box next to the player is the season this
            // episode belongs to — list exactly that season's episodes.
            const seasonAnchor = html.match(/<div class="Small--Box Season">\s*<a href="([^"]+)"/);
            if (seasonAnchor) {
                const seasonRes = await soraFetch(seasonAnchor[1]);
                if (seasonRes) {
                    const seasonHtml = await seasonRes.text();
                    collectEpisodesFromSeason(seasonHtml, results);
                }
            }
            // Fallback: the episode itself when no season listing is reachable.
            if (results.length === 0) {
                const numMatch = decodedUrl.match(/الحلقة\s*(\d+(?:\.\d+)?)/);
                results.push({
                    href: url,
                    number: numMatch ? parseFloat(numMatch[1]) : 1
                });
            }
        } else if (isMovie) {
            const watchMatch = html.match(/<a class="watch" href="([^"]+)"/);
            results.push({
                href: watchMatch ? watchMatch[1].trim() : url,
                number: 1
            });
        } else {
            // Series / season listing page: walk its season boxes, capped to keep
            // the request budget sane (One Piece has 26+ seasons).
            const seasonRegex = /<div class="Small--Box Season">\s*<a href="([^"]+)"/g;
            const seasonHrefs = [];
            let sm;
            while ((sm = seasonRegex.exec(html)) !== null) {
                if (seasonHrefs.indexOf(sm[1]) === -1) seasonHrefs.push(sm[1]);
            }
            const maxSeasons = 6;
            for (let i = 0; i < seasonHrefs.length && i < maxSeasons; i++) {
        ...
        console.log("extractEpisodes error: " + err.message);
...
async function extractStreamUrl(url) {
    try {
        ...
        if (responseText) {
            ...
            if (urlMatch) watchUrl = urlMatch[1];
        }

        const response = await soraFetch(watchUrl);
        ...
        for (const match of matches) {
            const url2 = "https://topcinema.io/wp-content/themes/movies2023/Ajaxat/Single/Server.php";

            const headers = {
                "Origin": "https://topcinema.io",
