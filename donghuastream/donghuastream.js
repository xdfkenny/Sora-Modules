async function searchResults(keyword) {
    const results = [];
    const response = await fetchv2(`https://donghuastream.org/?s=${keyword}`);
...
async function extractEpisodes(url) {
    ...
    const regex = /<a href="([^"]+)">.*?<div class="epl-num">(\d+)/gs;
    let match;
...
async function extractStreamUrl(url) {
    try {
        ...
        let iframeMatch = html.match(/dailymotion\.com\/embed\/video\/([a-zA-Z0-9]+)/);

        if (!iframeMatch) {
            iframeMatch = html.match(/geo\.dailymotion\.com\/player\/[a-z0-9]+\.html\?video=([a-zA-Z0-9]+)/);
        }
